...
private:
    ...
    // Runtime POI tracking (initialized from PointsOfInterest in BeginPlay)
    UPROPERTY(Transient)
    TArray<FPointOfInterestRuntimeInfo> PointOfInterestRuntimeInfos;

    // Room-level heat values updated each tick, separate from individual POI heat
    UPROPERTY(Transient)
    float PlayerHeatValue;
...
public:
    ...
    void AddActorToRoom(AActor* OtherActor);
    ...
    // Uses the POIHeatIncreaseOnCollect setting rather than a caller-specified amount
    UFUNCTION(BlueprintCallable)
    void AdjustPOIHeatCollect(int32 Index);
};