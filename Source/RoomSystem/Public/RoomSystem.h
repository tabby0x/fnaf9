...
public:
    ...
    // Called by ARoomAreaBase::BeginPlay / EndPlay
    UFUNCTION(BlueprintCallable)
    void RegisterRoom(ARoomAreaBase* Room);

    UFUNCTION(BlueprintCallable)
    void UnregisterRoom(ARoomAreaBase* Room);

    void RoomSystemTick();

    UFUNCTION(BlueprintCallable)
    void POIVisibility(bool bVisible);

    UFUNCTION(BlueprintCallable)
    void POIDetectionVisibility(bool bVisible);

    bool IsPOIVisible() const { return bPOIVisibility; }
    bool IsPOIDetectionVisible() const { return bPOIDetectionVisibility; }

private:
    UPROPERTY(Transient)
    TArray<ARoomAreaBase*> AllLoadedRooms;

    UPROPERTY(Transient)
    TArray<ARoomAreaBase*> PlayerCurrentRooms;

    UPROPERTY(Transient)
    TMap<AActor*, FPlayerRoomInfo> RoomPlayerExitTimes;

    FTimerHandle TimerHandle;

    UPROPERTY()
    float RoomUpdateTick;

    UPROPERTY()
    bool bPOIDetectionVisibility;
