...
public:
    ...
    UFUNCTION(BlueprintCallable, BlueprintPure)
    void HasMetConditions(AFNAFBasePlayerCharacter* BasePlayerCharacter, bool& bConditionsMet, EConditionFailReason& FailReason);
    ...
    UFUNCTION(BlueprintCallable, BlueprintPure)
    void CanPawnOpenDoor(APawn* Pawn, bool& bOutConditionsMet, EConditionFailReason& OutFailReason);

    UFUNCTION(BlueprintCallable)
    void CanPawnOpenDoorSide(APawn* Pawn, bool& bOutConditionsMet, EConditionFailReason& OutFailReason, EDoorSide Side);

private:
    void HasMetConditionsDoorSide(AFNAFBasePlayerCharacter* BasePlayerCharacter, bool& bConditionsMet, EConditionFailReason& FailReason, EDoorSide Side);

    /** Helper to check if pawn can open door - shared logic used by TickComponent, PawnEnteredDoor, etc. */
    bool CheckPawnConditions(APawn* Pawn, EDoorSide Side);

    /** Get the appropriate requirements based on game type */
    const FDoorEntryRequirements& GetActiveRequirements() const;

    /** Check if we're in Chowda (DLC/Ruin) mode */
    bool IsChowdaMode() const;
};