...
/*
 * Custom KeyFuncs for the mission state TSet -- keyed by FFNAFMissionState::Name (FName).
 * TSet element size is 40 bytes: 32 (FFNAFMissionState) + 8 (HashNextId + pad).
 */
struct MissionKeyFuncs : BaseKeyFuncs<FFNAFMissionState, FName, false>
{
    static const FName& GetSetKey(const FFNAFMissionState& Element) { return Element.Name; }
    static bool Matches(FName A, FName B) { return A == B; }
    static uint32 GetKeyHash(FName Key) { return GetTypeHash(Key); }
};
...
public:
    ...
    // Called from AMissionMarker::BeginPlay/EndPlay
    void RegisterMarker(AMissionMarker* Marker);
    void UnregisterMarker(AMissionMarker* Marker);
...
private:
    ...
    UPROPERTY()
    TArray<AMissionMarker*> MissionMarkers;

    /* Cannot be UPROPERTY (custom KeyFuncs). Serialized manually via ISaveHandlerInterface. */
    TSet<FFNAFMissionState, MissionKeyFuncs> MissionStates;

    FFNAFMissionState* FindMissionStateEntry(const FName& MissionName);
    const FFNAFMissionState* FindMissionStateEntry(const FName& MissionName) const;
    bool ShouldFilterForSurvival(const FFNAFMissionInfo& MissionInfo) const;
};
