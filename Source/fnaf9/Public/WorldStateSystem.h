...
public:
    ...
    bool bEnterExitFreddyEnabled;
    bool bCanCallFreddy;
    bool bPlayerInFreddy;
    bool bFreddySick;

    AFNAFBasePlayerCharacter* FreddyPawn;
    int32 FreddyPatrolPoint;
    FName FreddyPatrolPathName;
private:
    UPROPERTY(BlueprintReadWrite, EditAnywhere, Transient, meta = (AllowPrivateAccess = true))
    ...
    // Power station visit log
    TArray<FPowerStationSaveInfo> PowerStationInfo;

    // Power station state
    bool bPlayerInPowerStation;
    int32 PowerStationID;

    // Saves ActivatedObjects before chapter select reset
    TSet<FName> LoadedActivatedObjects;

    bool bCanUsePowerStation;
    bool bPlayerUsedHideSpot;
    bool bCanShowInstructionCards;
    bool bInstructionCardShown;

    TMap<int32, bool> DeactivatedCautionBots_Map;
    TSet<FName> BonnieBowlMMMJumpscareData;

    FName GoalPathName;

    int32 PlayerDeathCount;
    int32 PlayerSpottedCount;
    int32 SurvivalMaxDeaths;

    FVector LastSavedPlayerLocation;
    FRotator LastSavedPlayerRotation;
    bool FreddyWasInWorld;
    FVector LastSavedFreddyLocation;
    FRotator LastSavedFreddyRotation;
